...
|-------|-----------|------|
| **Object detection** | YOLOv5 (Ultralytics, PyTorch) | Locates the terminal position-number region on the template |
| **Text recognition** | Tesseract OCR (via `pytesseract`) | Reads the position numbers and returns their X, Y coordinates |
| **Image processing** | OpenCV, NumPy | Cropping, slicing, coordinate mapping, and drawing the red boxes |
